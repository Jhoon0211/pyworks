...
def process_guess(guess, word):
    ...
    global guessed_letters  # 추측된 단어를 모두 저장할 변수

    lives_remaining -= 1      # 기회가 1 줄어듬
    guessed_letters += guess  # 컴이 추측한 단어에 게이머가 입력한 글자가 추가됨
    if guess == word:
        return True # print("You win! Well done!")
    return False    # True 반환하면 바로 종료
